import {
  isRecord
} from "./chunk-FUSPHWLX.js";

// node_modules/get-it/dist/errorGuards-C36CfYld.js
...
function createRequester(options) {
  ...
  async function performFetch(fetchFn, opts) {
    ...
    let deadlines = totalDeadline === void 0 ? [] : [totalDeadline], timer2, controller;
    if (headersMs !== void 0) {
  ...
  async function getItBuffered(opts) {
    let { response, url, method, totalDeadline, clearDeadline } = await performFetch(opts.fetch ?? instanceFetch ?? globalThis.fetch, opts), httpErrors = opts.httpErrors ?? instanceHttpErrors ?? true;
    try {
  ...
  async function requestStream(opts) {
    ...
    async function getItStreamed(reqOpts) {
      let { response, url, method, totalDeadline, clearDeadline } = await performFetch(reqOpts.fetch ?? instanceFetch ?? globalThis.fetch, reqOpts), httpErrors = reqOpts.httpErrors ?? instanceHttpErrors ?? true;
      if (httpErrors && response.status >= 400) try {
        return await raceDeadline(bufferAndCheck(response, httpErrors, url, method), totalDeadline);
      } finally {
        clearDeadline();
      }
      return totalDeadline !== void 0 && clearDeadline(), capturedResponse = response, createBufferedResponse(response.status, response.statusText, response.headers, new Uint8Array(), response.url, response.redirected);
    }
...
function enabledMs(value) {
  if (!(value === void 0 || value === false || value <= 0)) return value;
}
function unrefTimer(timer2) {
  typeof timer2 == "object" && timer2 && "unref" in timer2 && typeof timer2.unref == "function" && timer2.unref();
}
function rejectAfterTimeout(ms, reason) {
  let timer2, deadline = new Promise((_, reject) => {
    timer2 = setTimeout(() => reject(reason), ms), unrefTimer(timer2);
  });
  return deadline.catch(() => {
  }), {
    deadline,
    clear: () => clearTimeout(timer2)
  };
}
...
function buildFetchArgs(opts, totalMs, instanceCredentials) {
  ...
  if (totalMs !== void 0) {
    let totalController = new AbortController(), timer2 = setTimeout(() => totalController.abort(new DOMException("The operation was aborted due to timeout", "TimeoutError")), totalMs);
    unrefTimer(timer2), clearTotalTimer = () => clearTimeout(timer2), signal = signal ? AbortSignal.any([signal, totalController.signal]) : totalController.signal;
  }
...
function isHttpError(error) {
  if (!isRecord2(error) || error.name !== "HttpError" || typeof error.message != "string" || typeof error.url != "string" || typeof error.method != "string" || typeof error.status != "number" || typeof error.statusText != "string" || !isHeaders(error.headers) || !("body" in error) || !isRecord2(error.response)) return false;
...
function isTimeoutError(error) {
  return !isRecord2(error) || error.name !== "TimeoutError" || typeof error.message != "string" ? false : error.code === void 0 || typeof error.code == "number" || error.code === "ETIMEDOUT" && typeof error.url == "string" && typeof error.method == "string" && typeof error.timeoutMs == "number" && error.phase === "headers";
}
function isRecord2(value) {
  return typeof value == "object" && !!value;
}
function isHeaders(value) {
  return isRecord2(value) && typeof value.get == "function";
}
...
var TimeoutError2 = createErrorClass(function(_super) {
...
function isRetryableRequest(error, _attemptNumber, options) {
  ...
  return method !== "GET" && method !== "HEAD" || error instanceof Error && "name" in error && error.name === "HttpError" ? false : isRetryableError(error);
}
...
function isRetryableError(error) {
  ...
  return code ? RETRYABLE_CODES.has(code) : error.name === "TimeoutError" || error.name === "AbortError" ? false : "retryable" in error && typeof error.retryable == "boolean" ? error.retryable : error instanceof TypeError;
}
...
function sleep(ms, signal) {
  return new Promise((resolve, reject) => {
    ...
    let timer2 = setTimeout(resolve, ms);
    signal?.addEventListener("abort", () => {
      clearTimeout(timer2), reject(signal.reason);
    }, { once: true });
  });
}

// node_modules/@sanity/client/dist/request-BhMuKj0D.js
var NEWLINE = /\r\n|[\n\r\u2028\u2029]/;
...
function getMarkerLines(loc, source) {
  ...
  if (lineDiff) for (let i = 0; i <= lineDiff; i++) {
    ...
    markerLines[lineNumber] = startColumn ? i === 0 ? [startColumn, source[lineNumber - 1].length - startColumn + 1] : i === lineDiff ? [0, endColumn] : [0, source[lineNumber - i].length] : true;
  }
...
function defineRequester(envOptions, config2 = {}) {
  ...
  }, observable2 = (options) => new Observable((subscriber) => {
...
function printWarnings(config2) {
  let seen = {}, shouldIgnore = (message) => config2.ignoreWarnings !== void 0 && (Array.isArray(config2.ignoreWarnings) ? config2.ignoreWarnings : [config2.ignoreWarnings]).some((pattern) => typeof pattern == "string" ? message.includes(pattern) : pattern.test(message));
  return { afterResponse(response) {
    let header = response.headers.get("x-sanity-warning");
    if (!header) return response;
    for (let msg of header.split(",").map((m) => m.trim())) !msg || seen[msg] || shouldIgnore(msg) || (seen[msg] = true, console.warn(msg));
    return response;
  } };
}
...
//# sourceMappingURL=chunk-JYLIBJH3.js.map
